...
export function delegateRequest({
  ...
  if (targetOperation === 'query' || targetOperation === 'mutation') {
    ...
    if (isPromise(executionResult)) {
      return executionResult.then(originalResultOrAsyncIterable => {
        if (isAsyncIterable(originalResultOrAsyncIterable)) {
          return asyncIterableToIncrementalResult(originalResultOrAsyncIterable, processedRequest, targetSchema).then(
            initialResult => {
    ...
    if (isAsyncIterable(executionResult)) {
      return asyncIterableToIncrementalResult(executionResult, processedRequest, targetSchema).then(initialResult => {
        return transformer.transformResult(initialResult);
      });
    } else {
